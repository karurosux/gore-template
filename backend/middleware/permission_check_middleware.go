...
func PermissionCheck(allowed []AllowedPermissions, next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		for i := 0; i < len(user.Role.Permissions); i++ {
			var permission permissionsdto.PermissionsDTO = (user.Role.Permissions)[i]
